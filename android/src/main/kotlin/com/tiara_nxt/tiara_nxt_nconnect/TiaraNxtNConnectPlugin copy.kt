...
//            rfidReader.connect()
            if (stringToRfidReaderMap.containsKey("")) {

            } else {
                val newRfidReader = rfidFactory.getRfidReader(make, mac, "android")
                newRfidReader.connect()
                newRfidReader.registerListener(rfidEventListener)
                stringToRfidReaderMap[""] = newRfidReader
            }
            true
        } catch (e: Exception) {
            println(e.toString())
...
//            val batteryLevel = rfidReader.batteryLevel
            ...
            if (stringToRfidReaderMap.containsKey(mac)) {
                val batteryLevel = stringToRfidReaderMap[mac]!!.batteryLevel
                return batteryLevel
            }
            return -1
        } catch (e: Exception) {
